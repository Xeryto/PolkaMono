        ...
        return (
          <div key={label} className="flex items-center flex-1 last:flex-none">
            <button
              ...
              className={`flex flex-col items-center gap-1.5 ${isClickable ? "cursor-pointer" : "cursor-default opacity-50"}`}
            >
              <div
              ...
              <span className={`text-xs whitespace-nowrap hidden sm:block ${isCurrent ? "text-foreground font-medium" : "text-muted-foreground"}`}>
                {label}
              </span>
            </button>

            {i < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 ${isCompleted ? "bg-brand/60" : "bg-muted"}`} />
            )}
          </div>
        );
      })}
