...
export default function App() {
  ...
  const navReducer = (state: AppNavState, action: AppNavAction): AppNavState => {
    switch (action.type) {
      case 'SET_PHASE':
      ...
      case 'TRANSITION':
        return { phase: action.phase, overlay: action.overlay ?? state.overlay };
      default:
        return state;
    }
  };

  const [navState, dispatchNav] = useReducer(navReducer, { phase: 'boot', overlay: 'static' });
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!isAuthenticated) {
        console.log('App - Unauthenticated at startup, clearing cart and going to Welcome.');
  ...
  const handleLogin = async () => {
    try {
      ...
      if (!user.is_email_verified) {
        console.log('User email not verified after login, showing verification screen');
  ...
  return (
    <GestureHandlerRootView style={{flex: 1}}>
      ...
      {navState.overlay === 'up' && <AuthLoadingScreen onFinish={handleAuthLoadingFinish} />}
      {navState.overlay === 'down' && <LoadingScreen onFinish={handleLoadingFinish} />}
