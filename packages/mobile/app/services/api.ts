...
class SessionManager {
  ...
  async getCurrentUser(): Promise<UserProfile | null> {
    ...
    if (
      ...
      now - this.userProfileCacheTime < this.USER_CACHE_DURATION
    ) {
...
export const requestVerificationEmail = async (): Promise<{
  message: string;
}> => {
...
export const checkEmailAvailability = async (
  ...
  try {
    const response = await apiRequest(
      `/api/v1/auth/check-email/${encodeURIComponent(email)}`,
      "GET",
      null,
      false,
    );
...
export const getProfileCompletionStatus =
  async (): Promise<ProfileCompletionStatus> => {
    ...
    const completionPromise = apiRequest(
      "/api/v1/user/profile/completion-status",
      "GET",
      undefined,
      true,
      {
...
export const getAvatarPresignedUrl = async (
  ...
  return await apiRequest(
    "/api/v1/user/upload/presigned-url",
    "POST",
    {
...
export const uploadToPresignedUrl = async (
  ...
  if (!putResponse.ok) {
    ...
    throw new ApiError(
      `Upload failed: ${putResponse.status} ${text}`,
      putResponse.status,
    );
  }
};
...
const loadFromStorage = async <T>(
  key: string,
  maxAge: number,
): Promise<{ data: T; time: number } | null> => {
  try {
...
export const clearFriendsCache = () => {
  ...
  sentRequestsCacheTime = 0;
  receivedRequestsCache = null;
  receivedRequestsCacheTime = 0;
};

// Friend recommendations cache (5min TTL, per friend)
const friendRecommendationsCache = new LRUCache<Product[]>(20, 5 * 60 * 1000);
export const clearFriendRecommendationsCache = () =>
  friendRecommendationsCache.clear();
...
export const getBrands = async (): Promise<Brand[]> => {
  ...
  if (!brandsCache) {
    const stored = await loadFromStorage<Brand[]>(
      ASYNC_BRANDS_KEY,
      ASYNC_CACHE_DURATION,
    );
    if (stored) {
  ...
  const brandsPromise = apiRequest("/api/v1/brands", "GET", undefined, false)
    .then((brands) => {
      brandsCache = brands;
...
export const getStyles = async (): Promise<Style[]> => {
  ...
  if (!stylesCache) {
    const stored = await loadFromStorage<Style[]>(
      ASYNC_STYLES_KEY,
      ASYNC_CACHE_DURATION,
    );
    if (stored) {
  ...
  const stylesPromise = apiRequest("/api/v1/styles", "GET", undefined, false)
    .then((styles) => {
      stylesCache = styles;
...
export const getSentFriendRequests = async (): Promise<FriendRequest[]> => {
  ...
  if (
    sentRequestsCache &&
    now - sentRequestsCacheTime < FRIENDS_CACHE_DURATION
  ) {
...
export const getReceivedFriendRequests = async (): Promise<FriendRequest[]> => {
  const now = Date.now();
  if (
    receivedRequestsCache &&
    now - receivedRequestsCacheTime < FRIENDS_CACHE_DURATION
  ) {
...
export const validatePasswordResetCode = async (
  ...
  await apiRequest(
    "/api/v1/auth/validate-password-reset-code",
    "POST",
    { identifier, code },
    false,
  );
...
export const simulateResetPassword = async (
...
export const getCategories = async (): Promise<any[]> => {
  ...
  if (!categoriesCache) {
    const stored = await loadFromStorage<any[]>(
      ASYNC_CATEGORIES_KEY,
      ASYNC_CACHE_DURATION,
    );
    if (stored) {
  ...
  const categoriesPromise = apiRequest(
    "/api/v1/categories",
    "GET",
    undefined,
    false,
...
export const createPayment = async (
  paymentDetails: PaymentCreateRequest,
...
export const getPaymentStatus = async (
  ...
  return await apiRequest(
    `/api/v1/payments/status?payment_id=${paymentId}`,
    "GET",
  );
};
...
export const getUserFavorites = async (
  ...
  if (
    !forceRefresh &&
    favoritesCache &&
    now - favoritesCacheTime < FAVORITES_CACHE_DURATION
  ) {
  ...
  const promise = apiRequest("/api/v1/user/favorites", "GET")
    .then((favorites) => {
...
export const getFriendRecommendations = async (
  ...
  const result = await apiRequest(
    `/api/v1/recommendations/for_friend/${friendId}`,
    "GET",
  );
...
export const getPopularItems = async (
  ...
  if (
    popularItemsCache &&
    now - popularItemsCacheTime < POPULAR_ITEMS_CACHE_DURATION
  ) {
    return popularItemsCache;
  }
  // Try AsyncStorage before API
  if (!popularItemsCache) {
    const stored = await loadFromStorage<Product[]>(
      ASYNC_POPULAR_KEY,
      ASYNC_CACHE_DURATION,
    );
    if (stored) {
      ...
      if (now - stored.time < POPULAR_ITEMS_CACHE_DURATION)
        return popularItemsCache;
    }
  }
  ...
  const popularPromise = apiRequest(
...
export const trackUserSwipe = async (
  swipeData: SwipeTrackingRequest,
): Promise<{ message: string }> => {
...
export const syncSwipeCountFromApi = async (
  apiCount: number,
): Promise<void> => {
  try {
...
export const trackSwipeWithOptimisticUpdate = async (
  swipeData: SwipeTrackingRequest,
