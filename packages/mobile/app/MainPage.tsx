...
const ExpandableSection: React.FC<{ title: string; content: string }> = ({ title, content }) => {
  ...
  return (
    <View style={styles.expandableContainer}>
      ...
      <RNAnimated.View style={{ maxHeight: contentHeight, paddingHorizontal: 10, paddingBottom: 10 }}>
...
const fetchMoreCards = async (count: number = 2): Promise<Product[]> => {
  try {
    ...
    if (!products || !Array.isArray(products)) {
      console.error('fetchMoreCards - API did not return a valid array of products.');
    ...
    return products.slice(0, count).map((p: api.Product, i: number): Product => ({
      ...
      price: p.price,
      images: (p.images && p.images.length > 0) ? p.images.map(img => ({ uri: img })) : [fallbackImage, vision2Image], // Use images from product
      isLiked: p.is_liked === true,
      variants: p.variants || [], // Use API data for variants
      description: p.description || `This is a detailed description for ${p.name}. It is crafted with the highest quality materials and designed to be both stylish and comfortable.`,
...
const MainPage = ({ navigation, route }: MainPageProps) => {
  ...
  const handleImagePress = (card: Product, x: number) => {
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_, gestureState) => {
        ...
        return !isAnimating && !isRefreshing && !isFlipped && Math.abs(gestureState.dy) > 5;
      },
  ...
  const renderFrontOfCard = useCallback((card: Product, index: number) => {
      ...
      <>
        <View style={styles.imageHolder}>
          ...
          <Pressable 
            ...
            onPress={(event) => handleImagePress(card, event.nativeEvent.locationX)}
          >
            <Image 
  ...
  const renderBackOfCard = useCallback((card: Product) => {
    ...
    return (
      <View style={styles.cardBackContainer}>
        ...
        <View style={styles.cardBackHeader}>
          <Image source={card.images[0]} style={styles.cardBackImage} resizeMode="contain" />
  ...
  const renderCard = useCallback((card: Product, index: number) => {
    ...
    console.log(`renderCard - Rendering card ${card.id} at index ${index}, isFlipped: ${isFlipped}, imagesCount: ${Array.isArray(card.images) ? card.images.length : 0}`);
      ...
      >
        ...
        <RNAnimated.View 
            style={[
                ...
                { backfaceVisibility: 'hidden', zIndex: frontZIndex, pointerEvents: frontPointerEvents },
                frontAnimatedStyle
            ]}
        >
        ...
        <RNAnimated.View 
            style={[
                StyleSheet.absoluteFill,
                { backfaceVisibility: 'hidden', zIndex: backZIndex, pointerEvents: backPointerEvents },
                backAnimatedStyle
            ]}
        >
            <View style={styles.cardFace}>
                {renderBackOfCard(card)}
            </View>
        </RNAnimated.View>
      </RNAnimated.View>
    );
...
const styles = StyleSheet.create({
  ...
  removeButton: { width: 25, height: 25, borderRadius: 7, backgroundColor: 'rgba(230, 109, 123, 0.54)', justifyContent: 'center', alignItems: 'center', position: 'absolute', right: 25, top: 25 },
  cardBackHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 20,
